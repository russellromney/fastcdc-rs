//! Reproducible chunking benchmarks for the vendored FastCDC.
//!
...
//!   new_api      — zero-alloc callback paths (only present after the patch)
...
fn run_for_each_chunk(data: &[u8], min: usize, avg: usize, max: usize) -> usize {
    let chunker = v2020::Chunker::new(min, avg, max);
    let mut acc = 0usize;
    chunker.for_each_chunk(data, |_off, length, hash, bytes| {
        acc ^= length ^ (hash as usize) ^ (bytes.len());
    });
    acc
}

fn run_for_each_boundary(data: &[u8], min: usize, avg: usize, max: usize) -> usize {
    let chunker = v2020::Chunker::new(min, avg, max);
    let mut acc = 0usize;
    chunker.for_each_boundary(data, |_off, length, hash| {
        acc ^= length ^ (hash as usize);
    });
    acc
}

fn bench_new_api(c: &mut Criterion) {
    let data = gen_random(8 * MIB, 99);
    let (min, avg, max) = sizes(16 * 1024);
    let mut g = c.benchmark_group("new_api");
    g.throughput(Throughput::Bytes(data.len() as u64));
    g.bench_function("iterator", |b| {
        b.iter(|| black_box(run_v2020_iter(black_box(&data), min, avg, max)))
    });
    g.bench_function("for_each_chunk", |b| {
        b.iter(|| black_box(run_for_each_chunk(black_box(&data), min, avg, max)))
    });
    g.bench_function("for_each_boundary", |b| {
        b.iter(|| black_box(run_for_each_boundary(black_box(&data), min, avg, max)))
    });
    g.finish();
}
